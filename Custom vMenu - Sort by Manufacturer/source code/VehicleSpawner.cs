            ...
            if (IsAllowed(Permission.VSAddon))
            {
                if (AddonVehicles != null)
                {
                    if (AddonVehicles.Count > 0)
                    {
                        ...
                        Menu ManuMenu = new Menu("Manufacturer List", "Choose your brand");
                        ...
                        {
                            Menu brandMenu = new Menu(brands.Key, "Select a model");
                            MenuItem brandBtn = new MenuItem(brands.Key, $"Spawn an vehicle from {brands.Key}") { Label = "→→→" };
                        ...
                        {
                            CaterMenu.AddMenuItem(unavailableCarsBtn);
